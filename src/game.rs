...
impl Game {
    pub fn new(stdout: Stdout, width: u16, height: u16) -> Self {
        ...
        Self { // Set the game data 
            stdout,
            original_terminal_size,
            width,
            height,
            food: None,
            snake: Snake::new( // set snake equal to a new snake struct 
                Point::new(width / 2, height / 2), // start in the center of the terminal
                3,
                ...
                if let Some(command) = self.get_command(interval - now.elapsed()) {
                    match command {
                        ...
                        Command::Turn(towards) => {
                            // If snake is moving parallel to direction
    ...
    fn place_food(&mut self) {
        loop {
            ...
            if !self.snake.contains_point(&point) { // If the snake does not contain the point where the food is being set
    ...
    fn calculate_interval(&self) -> Duration {
        ...
        Duration::from_millis(
            (MIN_INTERVAL + (((MAX_INTERVAL - MAX_INTERVAL) / MAX_SPEED) * speed)) as u64
        )
    }

    fn get_command(&self, wait_for: Duration) -> Option<Command> {
        let key_event = self.wait_for_key_event(wait_for)?; // Wait until the player does something or the wait_for duration elapses
    ...
    fn has_collided_with_wall(&self) -> bool {
        ...
        match self.snake.get_direction() {
            Direction::Up => head_point.y == 0, // snake head is 0
            Direction::Right => head_point.x == self.width - 1, // snake head is less than the width of the grid
            Direction::Down => head_point.y == self.height - 1, // snake head is less than the height of the grid
            Direction::Left => head_point.x == 0, // snake head is 0
        }
    }

    fn has_bitten_itself(&self) -> bool {
        let next_head_point = self.snake.get_head_point().transform(self.snake.get_direction(), 1); // get the next point of the head on direction change
        let mut next_body_points = self.snake.get_body_points().clone(); // get next point of body on direction change
        next_body_points.remove(next_body_points.len() - 1); // removing one point of body to account for head 
        next_body_points.remove(0);

        next_body_points.contains(&next_head_point) // check if head of snake is in body
    }
